...
const CalendarEvents: React.FC<CalendarEventsProps> = ({ user }) => {
  ...
  const handleShareAgenda = async (monthTitle: string, monthEvents: ChurchEvent[]) => {
      ...
      sortedEvents.forEach(event => {
          ...
          const categoryEmoji = { culto: '🙏', simposio: '🎓', confraternizacao: '🎉', reuniao: '💼', outro: '🗓️' }[event.category] || '🗓️';
      ...
      if (navigator.share) {
          try {
              await navigator.share({ title: `Agenda ${monthTitle}`, text: text });
    ...
    <div className="space-y-6 animate-fade-in pb-20">
       <div className={`relative rounded-2xl shadow-lg overflow-hidden group min-h-[180px] flex flex-col justify-center transition-all ${!bannerUrl ? 'bg-gradient-to-r from-blue-700 to-indigo-800' : ''}`}>
          ...
          {isAdmin && (
            <div className="absolute bottom-4 right-4 z-20 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
               ...
               {bannerUrl && (
                  <button onClick={handleRemoveBanner} className="bg-red-500/80 hover:bg-red-600 text-white p-2.5 rounded-full backdrop-blur-sm transition-all shadow-md border border-white/10" title="Remover Banner">
                     <Eraser size={18} />
                  </button>
               )}
               <button onClick={() => bannerInputRef.current?.click()} className="bg-blue-600/80 hover:bg-blue-600 text-white p-2.5 rounded-full backdrop-blur-sm transition-all shadow-md border border-white/10" title="Alterar Imagem de Capa">
                  <Camera size={18} />
               </button>
            </div>
          )}
       ...
       <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex bg-white p-1 rounded-xl shadow-sm border border-gray-200 w-full sm:w-auto">
             <button type="button" onClick={() => setFilterScope('all')} className={`flex-1 sm:flex-none px-4 py-2 text-sm font-medium rounded-lg transition-all ${filterScope === 'all' ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}>Todos</button>
             <button type="button" onClick={() => setFilterScope('campo')} className={`flex-1 sm:flex-none px-4 py-2 text-sm font-medium rounded-lg transition-all ${filterScope === 'campo' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}>Campo ADMSJP</button>
             <button type="button" onClick={() => setFilterScope('local')} className={`flex-1 sm:flex-none px-4 py-2 text-sm font-medium rounded-lg transition-all ${filterScope === 'local' ? 'bg-emerald-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}>Local</button>
          </div>
          {isAdmin && (
            <button type="button" onClick={() => handleOpenModal()} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-md transition-all active:scale-95">
       ...
       <div className="space-y-8">
          {Object.keys(groupedEvents).length === 0 ? (
            ...
            Object.keys(groupedEvents).map((monthYear, index) => (
                <div key={monthYear} className="space-y-4 animate-fade-in" style={{ animationDelay: `${index * 150}ms`, opacity: 0, animationFillMode: 'forwards' }}>
                    <div className="sticky top-0 bg-gray-50/95 backdrop-blur-sm z-10 py-2 px-3 rounded-lg border-l-4 border-blue-500 flex justify-between items-center shadow-sm">
                        <h3 className="text-lg font-bold text-gray-700 pl-2">{monthYear}</h3>
                        <button onClick={() => handleShareAgenda(monthYear, groupedEvents[monthYear])} className="flex items-center gap-2 text-blue-600 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg transition-colors text-sm font-bold">
                                    ...
                                    <div className="p-5 flex-1">
                                        <div className="flex justify-between items-start mb-3">
                                            ...
                                            <div className="flex gap-1">
                                                <button type="button" onClick={() => handleShareEvent(event)} className="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-full transition-colors" title="Compartilhar este evento">
                                                    <Share2 size={16}/>
                                                </button>
                                                {isAdmin && <button type="button" onClick={() => handleOpenModal(event)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-full transition-colors"><Edit2 size={16}/></button>}
                                                {isAdmin && <button type="button" onClick={() => handleDeleteClick(event.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-full transition-colors"><Trash2 size={16}/></button>}
                                            </div>
                                        </div>
                                        <div className="mb-2">
       ...
       {itemToDelete && isAdmin && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 text-center border border-red-100">
             ...
             <div className="flex gap-3">
                <button type="button" onClick={() => setItemToDelete(null)} className="flex-1 px-4 py-3 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 font-bold transition-colors">
                  Cancelar
                </button>
                <button type="button" onClick={confirmDelete} className="flex-1 px-4 py-3 bg-red-600 text-white rounded-xl hover:bg-red-700 font-bold shadow-md transition-all active:scale-95">
                  Confirmar
                </button>
             </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 relative overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar">
               <div>
                  ...
                  <input value={formData.title || ''} onChange={e => setFormData({...formData, title: e.target.value})} placeholder="Ex: Simpósio de Doutrina" className="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 outline-none" />
               </div>
               <div className="grid grid-cols-2 gap-4">
                   <div>
                       <label className="block text-sm font-bold text-gray-700 mb-1">Origem</label>
                       <select value={formData.scope} onChange={e => setFormData({...formData, scope: e.target.value as any})} className="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 outline-none">
                   ...
                   <div>
                       ...
                       <select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value as any})} className="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 outline-none">
               ...
               <div className="grid grid-cols-2 gap-4">
                   <div>
                       ...
                       <input type="date" value={formData.date || ''} onChange={e => setFormData({...formData, date: e.target.value})} className="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 outline-none" />
                   </div>
                   <div>
                       <label className="block text-sm font-bold text-gray-700 mb-1">Horário</label>
                       <input type="time" value={formData.time || ''} onChange={e => setFormData({...formData, time: e.target.value})} className="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 outline-none" />
                   </div>
               </div>
               <div>
                  ...
                  <div className="relative">
                      ...
                      <input value={formData.location || ''} onChange={e => setFormData({...formData, location: e.target.value})} placeholder="Ex: Templo Sede" className="w-full border border-gray-300 rounded-xl p-3 pl-10 focus:ring-2 focus:ring-blue-500 outline-none" />
                  </div>
               </div>
               <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Descrição / Detalhes</label>
                  <textarea rows={3} value={formData.description || ''} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="Informações adicionais sobre o evento..." className="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 outline-none resize-none" />
               </div>
            </div>
            <div className="pt-6 mt-2 border-t border-gray-100 flex gap-3">
               <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-3 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200 transition-colors">Cancelar</button>
               <button type="button" onClick={handleSave} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg hover:shadow-xl transition-all active:scale-95">Salvar Evento</button>
            </div>
          </div>
        </div>
       )}
